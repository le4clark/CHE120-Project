...
while True:
    ...
    if not gameover:
        ...
        #prevent movement ~AS
        if p1.health <= 0:
            #another check for health just in case something decides to break ~AS
            if p1Facing:
                p1.pos = p1.pos.move(3, -3)
                animation= p1.p1JumpIdleAnim[animFrame]
                #Facing right fly off screen ~AS
            else:
                p1.pos = p1.pos.move(-3, -3)
                animation= pygame.transform.flip(p1.p1JumpIdleAnim[animFrame], True, False)
                #The previous thing but left ~AS
        elif p2.health <= 0:
            #another check for health just in case something decides to break ~AS
            if p2Facing:
                p2.pos = p2.pos.move(3, -3)
                animation2= p2.p2JumpIdleAnim[animFrame]
                #Facing right fly off screen ~AS
            else:
                p2.pos = p2.pos.move(-3, -3)
                animation2= pygame.transform.flip(p2.p2JumpIdleAnim[animFrame], True, False)
                #The previous thing but left ~AS
        gameEndTimer -= 1
        #countdown to exit game ~AS
    if gameEndTimer <= 0:
        pygame.quit()
        #ends pygame ~AS
        sys.exit()
